...
if __name__ == '__main__':
    ...
    if crc != 0x4C6B:
        ...
        print "Ok"
    """
    print "test dataframe req #1:",
    st = "\x01\x18\x00\x09\x00\x01\x09\x00\x00\x00\x00\x01\x00\x02\x00\x00\x00\x02\x00\x00\xe1\x07\x04\x13\x15\x04\x03"
    # DF1 uses same algorithm - just starts with CRC=0x0000 instead of 0xFFFF
    # note: <DLE><STX> and the <DLE> of the <DLE><ETX> pair NOT to be included
    print "st len is: ", len(st)
    crc = calcString( st, INITIAL_MODBUS)
    #print "crc is %x" %crc
    #crc = calcByte( "\x03", crc) # final ETX added
    if crc != 0xa101:
        print "BAD - ERROR - FAILED!",
        print "expect:0xa101 but saw 0x%x" % crc
    else:
        print "Ok"
 

    print "test dataframe reply #2:",
    st = "\x01\x11\x00\x09\x00\x01\x09\xe1\x07\x04\x13\x15\x04\x03\xe8\x03\x00\x00\x00\x00"
    # DF1 uses same algorithm - just starts with CRC=0x0000 instead of 0xFFFF
    # note: <DLE><STX> and the <DLE> of the <DLE><ETX> pair NOT to be included
    print "st len is: ", len(st)
    crc = calcString( st, INITIAL_MODBUS)
    ...
    if crc != 0x6400:
        print "BAD - ERROR - FAILED!",
        print "expect:0x6400 but saw 0x%x" % crc
    else:
        print "Ok"
